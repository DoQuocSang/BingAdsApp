...
namespace BingAdsDemo.BingAds
{
    ...
    public class ReportRequests : ExampleBase
    {
        ...
        /// </summary>
        protected const string FileDirectory = @"C:\Users\vu.pham\Downloads\";

        /// <summary>
        /// The name of the report file.
        /// </summary>
        //protected string ResultFileName = @"result." + ReportFileFormat.ToString().ToLower();
        protected string ResultFileName = $"result_{DateTime.UtcNow:yyyyMMdd_HHmmss}.{ReportFileFormat.ToString().ToLower()}";

        /// <summary>
        /// The maximum amount of time (in milliseconds) that you want to wait for the report download.
        /// In this demo, it is 6 minutes.
        /// </summary>
        ...
        public async override Task RunAsync(AuthorizationData authorizationData)
        {
            try
            {
                ...
                //OutputStatusMessage($"Request ID: {requestId}");
                ...
                //var requestId = "315752266184110081_1747812461527";
        ...
        private async Task SubmitAndDownloadAsync(ReportRequest reportRequest)
        {
            ...
            {
                Console.WriteLine($"[{DateTime.Now}] Checking report status... Attempt {i + 1}");

                Thread.Sleep(waitTime);

                reportingOperationStatus = await reportingDownloadOperation.GetStatusAsync();

                Console.WriteLine($"Current status: {reportingOperationStatus.Status}");

                if (reportingOperationStatus.Status == ReportRequestStatusType.Success)
                {
                    Console.WriteLine("Report is ready to download.");
                    break;
                }
                else if (reportingOperationStatus.Status == ReportRequestStatusType.Error)
                {
                ...
                {
                    Console.WriteLine("Timeout reached. Report is still not ready.");
                }
            }
        ...
        private async Task DownloadReportAsync(ReportingDownloadParameters reportingDownloadParameters)
        {
            ...
            Console.WriteLine($"Saving file to: {ReportingServiceManager.WorkingDirectory}/{reportingDownloadParameters.ResultFileName}");
